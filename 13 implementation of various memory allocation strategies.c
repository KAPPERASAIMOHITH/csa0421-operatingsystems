...
#define BLOCK_COUNT 10    // Number of memory blocks
...
void initialize_memory() {
    for (int i = 0; i < BLOCK_COUNT; i++) {
        memory[i].size = (rand() % 100) + 10;  // Random block size between 10 and 109
        memory[i].is_free = 1;  // Mark all blocks as free initially
    }
}
...
// Print the status of memory blocks
